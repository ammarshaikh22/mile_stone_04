...
export default function CreateBlogPage() {
    ...
    const updateSubSection = (index: number, field: keyof SubSection, value: string) => {
        const updated = subSections.map((section, i) => (i === index ? { ...section, [field]: value } : section))
            ...
            <div className="max-w-[90%] mx-auto py-8">
                <div className="space-y-8">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="border-2 border-dashed border-slate-600 rounded-lg p-12 text-center hover:border-slate-500 transition-colors">
                            ...
                            <label htmlFor="featured-image" className="cursor-pointer">
                                ...
                                <p className="text-slate-400 text-sm">PNG, JPG, GIF, WEBP, SVG ...</p>
                            </label>
                            {featuredImage && <p className="text-green-400 mt-2">Selected: {featuredImage.name}</p>}
                        </div>
                    </div>
                    <div className="space-y-2">
                        <Input
                    ...
                    <div className="space-y-4">
                        <Label className="text-white font-medium">Add categories ({categories.length}/3)</Label>
                        <div className="flex flex-wrap gap-2 mb-2">
                            {categories.map((category) => (
                                <Badge key={category} variant="secondary" className="bg-[#1F2937] text-white">
                                    {category}
                                    <button onClick={() => removeCategory(category)} className="ml-2">
                                        <X className="w-3 h-3" />
                                    </button>
                                </Badge>
                            ))}
                        ...
                        {categories.length < 3 && (
                            <div className="flex gap-2 items-center">
                                ...
                                />
                                <Button onClick={addCategory} className="bg-[#1F2937] hover:bg-slate-500 text-white">
                    ...
                    <div className="space-y-4">
                        <div className="flex flex-wrap gap-2">
                            {tags.map((tag) => (
                                <Badge key={tag} variant="outline" className="bg-slate-700 border-slate-600 text-white">
                                    # {tag}
                    ...
                    <div className="border-t border-slate-600 pt-6">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <Button onClick={addSubSection} variant={'outline'} size="sm" className="bg-[#1F2937] text-white">
                            ...
                            {subSections.map((section, index) => (
                                <Card key={index} className="bg-[#0F172A] border-slate-600">
                                    <CardContent className="p-4 space-y-4">
                                        <div className="flex items-center justify-between">
                                            <Label className="text-white font-medium">Section {index + 1}</Label>
                                            <Button
                    ...
                    <div className="flex items-center gap-4 pt-6 border-t border-slate-600">
                        <Button onClick={handlePublish} className="bg-indigo-600 hover:bg-indigo-700 text-white px-8">
                            Publish
                        </Button>
                        <Button
